...
def UpBlockComp(in_planes, out_planes, size):
    block = nn.Sequential(
        nn.Upsample(scale_factor=2, mode='nearest'),
...
class CGenerator(nn.Module):
    def __init__(self, nz, ngf, nc, img_size, num_classes, skip_layer):
        ...
        self.init = InitLayer(nz + num_classes, self.nfc[4])
        ...
        self.embedding = nn.Embedding(self.num_classes, nz)
...
class Discriminator(nn.Module):
    def __init__(self, ndf, nc, img_size, num_classes, skip_layer):
        ...
        self.down_from_big = downBlockHead(nc + num_classes, self.nfc[self.img_size])
        self.down_from_small = nn.Sequential(
            spectral_norm(nn.Conv2d(nc + num_classes, self.nfc[256], 4, 2, 1, bias=False)),
            nn.LeakyReLU(0.2),
        ...
        self.embedding = nn.Embedding(num_classes, 25)
    ...
    def forward(self, input, input_128, class_label, label="fake", part=(0,0)):
        ...
        class_label = class_label.view(input.size(0),self.num_classes, 1, 1)
        class_label_128 = class_label.repeat(1, 1, 128, 128)
        class_label = class_label.repeat(1, 1, self.img_size, self.img_size)
        input = torch.cat([input, class_label], dim=1)
        input_128 = torch.cat([input_128, class_label_128], dim=1)
        ...
        #embedded_y = self.embedding(class_label).view(-1)
        #proj = torch.sum(rf * embedded_y)
        #proj_small = torch.sum(rf_small * embedded_y)
        #rf = rf + proj
        #rf_small = rf_small + proj_small

        if label == "real": 
            print(feature_small.size())
