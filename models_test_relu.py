...
class UpBlock(nn.Module):
    def __init__(self, in_planes, out_planes, size, z_dim, class_emb_dim):
        super().__init__()
...
class Generator(nn.Module):
    def __init__(self, nz, ngf, nc, img_size, num_classes, skip_layer):
        ...
        for i in self.nfc:
            if i < self.img_size:
                self.features.append(UpBlock(self.nfc[i], self.nfc[i*2], i*2, self.chunk_size, self.class_emb_dim))
...
class Discriminator(nn.Module):
    def __init__(self, ndf, nc, img_size, num_classes, skip_layer):
        ...
        self.embedding = nn.Linear(num_classes, self.nfc[16] * 8 * 8)
        self.embedding_small = nn.Linear(num_classes, self.nfc[32] * 8 * 8)
    ...
    def forward(self, input, input_128, class_label, label="fake", part=(0,0)):
        ...
        phi = feature.view(feature.size(0), -1)
        ...
        phi_small = feature_small.view(feature_small.size(0), -1)
